 ...
 * @usage:
 *   ./urdf-parse --file path/to/model.urdf [--log]
 */
 #include <iostream>
 ...
 int main(int argc, char* argv[]) {
    ...
    if (file_path.empty()) {
        std::cerr << "Error: No URDF file specified. Use --file <path>." << std::endl;
        return 1;
    }
    ...
    try {
        ...
        std::cout << "URDF model successfully parsed from: " << file_path << std::endl;
